<template>
    <form @submit.prevent="saveChanges">
        <card-page header-id="hdr_top_of_hour">
            ...
            <info-card>
                <p class="mb-0">
                    {{ $gettext('Queues a station ID at exactly :00 without interrupting the current song. During the lookahead window, music picks are filtered so long songs do not run past the hour. Tag files as ID on the Music Files page.') }}
                </p>
            </info-card>
            ...
            <loading :loading="isLoading" lazy>
                <div class="card-body">
                    ...
                    <form-group id="top_of_hour_finish_buffer_seconds" class="mb-3">
                        <template #label>
                    ...
                    <form-group id="top_of_hour_compliance_tolerance_seconds" class="mb-3">
                        <template #label>
                    ...
                    <p class="text-secondary small mb-3">
                        {{ $gettext('Hard Clock Trigger and Smart Ducking are managed separately on the Playout Controls page.') }}
                    </p>

                    <p class="text-secondary mb-3">
                        {{ $gettext('ID files in library: %{count}', {count: legalIdMediaCount}) }}
                    </p>
                        ...
                        <div class="row g-3 mb-3">
                            <div class="col-6 col-md-3">
                                <div class="border rounded p-2 text-center">
                                    ...
                                    <div class="small text-muted">{{ $gettext('On time') }}</div>
                                </div>
                            </div>
                            <div class="col-6 col-md-3">
                                <div class="border rounded p-2 text-center">
                                    <div class="fs-4 fw-semibold">{{ compliance?.on_time_count ?? 0 }}</div>
                                    <div class="small text-muted">{{ $gettext('Compliant hours') }}</div>
                                </div>
                            </div>
                            <div class="col-6 col-md-3">
                                <div class="border rounded p-2 text-center">
                                    <div class="fs-4 fw-semibold text-warning">{{ compliance?.late_count ?? 0 }}</div>
                                    <div class="small text-muted">{{ $gettext('Late (> tolerance)') }}</div>
                                </div>
                            </div>
                            <div class="col-6 col-md-3">
                                <div class="border rounded p-2 text-center">
                                    <div class="fs-4 fw-semibold text-secondary">{{ compliance?.fallback_count ?? 0 }}</div>
                                    <div class="small text-muted">{{ $gettext('Fallback events') }}</div>
                                </div>
                            </div>
                        </div>
                    </template>
